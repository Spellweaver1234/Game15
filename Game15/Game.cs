...
namespace Game15
{
    class Game
    {
        ...
        public void Start()
        {
            for (int x = 0; x < size; x++)
                for (int y = 0; y < size; y++)
                    map[x, y] = CoordsToPositions(x, y) + 1;
            ...
            for (int x = 0; x < size; x++)
                for (int y = 0; y < size; y++)
                    if (!(x == size - 1 && y == size - 1))
                        if (map[x, y] != CoordsToPositions(x, y) + 1)
                            return false;
            return true;
        }
        ...
        private void PositionsToCoordinates(int position, out int x, out int y)
        {
